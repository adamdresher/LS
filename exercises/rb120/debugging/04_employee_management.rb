...
=begin
An exception is raised on line 83 when evaluating if `raul` exists within
`miller_contracting#employees`.  This exception is due to `Employee#==`
referencing a private method `serial_number` for a different object.  Private
methods are available only to the calling instance object.
To make `serial_number` available to `Employee#==`, `serial_number` can be made
a `public` or `protected` method.  Due to the sensitive data that is implied by
the name, `serial_number` would be best placed under `protected`.  This makes
an instance of `Employee` have access to the `serial_number` of other `Employee`
instances (within the class definition).
=end
