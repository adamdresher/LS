...
However, this approach limits the program's ability to catch `NaN` errors.  A
`NaN` error will occur when both `positive` and `negative` return `0.0`,
resulting in the expression `positive / (positive + negative` evaluating to
`0.0 / 0.0`.  To handle this, the `NoScore` class is added back to `DoesItRock`,
as well as returning `when NoScore; "No idea about #{term}..."` back to the
`case` block.  The `Exception` in the guard cluse on line 33 must be replaced
with the correct error, `ZeroDivisionError`.  Finally, `ZeroDivisionError` must
also be raised before the rescue, in order for it to be rescued.
...
module DoesItRock
  ...
  class Score
    def self.for_term(term)
      ...
      raise ZeroDivisionError unless score.finite?
